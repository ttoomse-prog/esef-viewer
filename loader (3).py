...
def _extract_semantic_html(html_str: str) -> list:
    ...
    content_tags = {"p", "li"}
    block_tags = {"div", "section", "article", "aside", "main",
                  "p", "li", "h1", "h2", "h3", "h4", "h5",
                  "b", "strong", "span"}
    ...
    def flush():
        ...
        if len(text) > 60:   # skip trivially short fragments
            rows.append({
                "section":    current_section,
                "heading":    current_heading or "",
                "seq":        seq,
    ...
    for element in body.descendants:
        ...
        # Skip elements that contain other block elements (avoid double-counting)
        has_block_child = any(
            c.name in block_tags
        ...
        if _looks_like_heading(element, has_semantic_headings):
            flush()
            current_chunks = []
            current_heading = text
            classified = _classify_section(text)
            ...
            # Don't reset section on unclassified headings — inherit last known
        elif element.name in content_tags or (
            not has_semantic_headings and element.name in {"div", "span", "p"}
        ):
            # Filter out pure-numeric / symbol fragments (leftover table noise)
            if re.match(r"^[\d.,£€$%\s\-–—()\[\]/n.a]+$", text, re.I) and len(text) < 60:
                continue
            if len(text) > 30:
